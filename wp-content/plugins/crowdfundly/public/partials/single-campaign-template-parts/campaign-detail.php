<div class="campaign__details">
	...
	<div class="campaign__actions">
		...
		if ( isset( $campaign->validity_countdown_days ) && $campaign->validity_countdown_days == 0 ) {
			$button_default_text = __( 'Target Date Exceed', 'crowdfundly' );
		}
		
		if ( $donation_status == 'disabled' ) $btn_id = '';
		...
		if ( ! empty( $organization_gateways ) ) :
			...
			?>
				<button <?php echo esc_attr( $donation_status ); ?> id="<?php echo esc_attr( $btn_id ); ?>" type="button" data-toggle="modal" class="btn btn-primary btn-block campaign__actions-btn">
			...
			<?php else :
				...
				$url = $donation_status == 'disabled' ? '' : Crowdfundly_Settings::getDonationPagePermalink($campaign->slug);
				?>
				<a href="<?php echo esc_url( $url ); ?>">
					<button <?php echo esc_attr( $donation_status ); ?> type="button" data-toggle="modal" class="btn btn-primary btn-block campaign__actions-btn">
						<?php echo esc_html( get_theme_mod( 'cf_single_campn_donation_btn_texts', $button_default_text ) ); ?>
					</button>
				</a>
			<?php
